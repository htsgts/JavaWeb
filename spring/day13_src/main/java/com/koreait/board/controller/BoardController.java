...
import com.koreait.board.bean.vo.PageDTO;
...
//		log.info(MyUtil.getColor() + "getList : " + type + "-" + keyword + MyUtil.END);
		log.info(MyUtil.getColor() + "getList : " + cri + MyUtil.END);
...
//		model.addAttribute("board", dao.getListWithKey(type, keyword));
		dao.getListWithPaging(cri).forEach(board->log.info(MyUtil.getColor() + board + MyUtil.END));
		model.addAttribute("board", dao.getListWithPaging(cri));
		model.addAttribute("myFeel", "very gloomy");
		//model.addAttribute("page", new PageDTO(cri, 123));
		model.addAttribute("page", new PageDTO(cri, dao.getTotal(cri)));
